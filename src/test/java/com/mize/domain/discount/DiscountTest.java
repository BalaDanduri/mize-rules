...
public class DiscountTest  extends JPATest {
	...
	private Discount discountObjectToSave() {
		...
		discount.setCreatedDate(DateTime.now());
		discount.setUpdatedDate(DateTime.now());
		discount.setStartDate(DateTime.now());
		discount.setEndDate(DateTime.now().plusMonths(2));
	...
	public class DiscountRowMapper implements RowMapper<Discount>{
		...
		public Discount mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			discount.setCreatedDate(Formatter.dateTime(rs.getTimestamp("created_date")));
			discount.setUpdatedDate(Formatter.dateTime(rs.getTimestamp("updated_date")));
			discount.setStartDate(Formatter.dateTime(rs.getTimestamp("start_date")));
			discount.setEndDate(Formatter.dateTime(rs.getTimestamp("end_date")));
