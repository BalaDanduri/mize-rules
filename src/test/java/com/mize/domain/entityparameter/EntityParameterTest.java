...
public class EntityParameterTest extends JPATest{
	...
	private EntityParameter entityParamObjectToSave() {
		...
		entityParameter.setStartDate(DateTime.now());
		entityParameter.setEndDate(DateTime.now().plusMonths(2));
		entityParameter.setCreatedDate(DateTime.now());
		entityParameter.setUpdatedDate(DateTime.now().plusMonths(2));
	...
	private class EntityParameterRowMapper implements RowMapper<EntityParameter> {
		...
		public EntityParameter mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			entityParameter.setStartDate(Formatter.dateTime(rs.getTimestamp("start_date")));
			entityParameter.setEndDate(Formatter.dateTime(rs.getTimestamp("end_date")));
			entityParameter.setCreatedDate(Formatter.dateTime(rs.getTimestamp("created_date")));
			entityParameter.setUpdatedDate(Formatter.dateTime(rs.getTimestamp("updated_date")));
